...
namespace Gateway.Api.AsyncDataServices
{
    public class MessageBusSubscriber : BackgroundService
    {
        ...
        private void InitializeRabbitMQ()
        {
            ...
            var test1 = _configuration["RabbitMQHost"];
            var tets2 = int.Parse(_configuration["RabbitMQPort"]);

            _connection = factory.CreateConnection();
            _channel = _connection.CreateModel();
            _channel.ExchangeDeclare(exchange: "authResponse", type: ExchangeType.Fanout);
            _queueName = _channel.QueueDeclare().QueueName;
            _channel.QueueBind(queue: _queueName, exchange: "authResponse", routingKey: "");

            Console.WriteLine("--> Listening on the Message Bus(authResponse)...");
